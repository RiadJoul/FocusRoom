import React from 'react';
import { Text, View } from 'react-native';

interface BoardingPassCardProps {
  from: string;
  fromLabel?: string;
  to: string;
  toLabel?: string;
  durationLabel: string;
  isPremium?: boolean;
}

export function BoardingPassCard({
  from,
  fromLabel,
  to,
  toLabel,
  durationLabel,
  isPremium = false,
}: BoardingPassCardProps) {
  return (
    <View className="bg-white rounded-3xl px-6 pt-5 pb-6 shadow-2xl shadow-black/60">
      {/* Header */}
      <View className="flex-row items-center justify-between mb-4">
        <View>
          <Text className="text-[10px] text-gray-500 tracking-[2px] font-primary-medium">
            FOCUSROOM AIRWAYS
          </Text>
          <Text className="text-base font-primary-bold text-black">BOARDING PASS</Text>
        </View>
        {isPremium ? (
          <View
            style={{
              backgroundColor: '#A78BFA',
              paddingHorizontal: 12,
              paddingVertical: 6,
              borderRadius: 6,
              shadowColor: '#A78BFA',
              shadowOffset: { width: 0, height: 0 },
              shadowOpacity: 0.7,
              shadowRadius: 10,
              elevation: 10,
            }}
          >
            <Text className="text-white font-primary-bold text-sm tracking-wider">FIRST CLASS</Text>
          </View>
        ) : (
          <View className="bg-black px-3 py-1.5 rounded-lg">
            <Text className="text-white font-primary-bold text-sm tracking-wider">ECONOMY</Text>
          </View>
        )}
      </View>

      {/* Route row */}
      <View className="flex-row items-center justify-between mb-6">
        <View>
          <Text className="text-xs text-gray-500 mb-1">FROM</Text>
          <Text className="text-3xl font-primary-bold text-black">{from}</Text>
          {fromLabel ? (
            <Text className="text-xs text-gray-500">{fromLabel}</Text>
          ) : null}
        </View>
        <View className="items-center">
          <View className="flex-row items-center mb-1">
            <View className="w-2 h-2 rounded-full bg-black mr-1.5" />
            <View className="w-14 h-[1px] bg-gray-400" />
            <View className="w-2 h-2 rounded-full bg-black ml-1.5" />
          </View>
          <Text className="text-xs text-gray-500">{durationLabel}</Text>
        </View>
        <View className="items-end">
          <Text className="text-xs text-gray-500 mb-1">TO</Text>
          <Text className="text-3xl font-primary-bold text-black">{to}</Text>
          {toLabel ? (
            <Text className="text-xs text-gray-500">{toLabel}</Text>
          ) : null}
        </View>
      </View>

      {/* Dotted divider */}
      <View className="flex-row justify-between items-center mb-4">
        <View className="w-3 h-3 rounded-full bg-black/5" />
        <View className="flex-1 flex-row justify-between mx-2">
          {Array.from({ length: 24 }).map((_, idx) => (
            <View key={idx} className="w-2 h-[1px] bg-gray-300" />
          ))}
        </View>
        <View className="w-3 h-3 rounded-full bg-black/5" />
      </View>

      {/* Barcode */}
      <View className="flex-row flex-1 h-10">
        {Array.from({ length: 36 }).map((_, idx) => (
          <View
            key={idx}
            style={{
              flex: 1,
              backgroundColor: idx % 2 === 0 ? '#000' : '#fff',
              marginHorizontal: idx % 3 === 0 ? 2 : 1,
            }}
          />
        ))}
      </View>
    </View>
  );
}
